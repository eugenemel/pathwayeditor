...
public:
    ...
    inline float getConcentration(int idx) { return concentrations[idx]; }
    void setConcentration( int idx, float v) { if(concentrations.size() <= idx) concentrations.resize(idx+1); concentrations[idx]=v; }
    ...
    float setNodeSize(float x) { _nodeSize=x; }
