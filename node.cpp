...
Node::Node(QGraphicsItem* parent, QGraphicsScene *scene):QGraphicsItem(parent,scene)
{
    ...
    setConcentration(0,100);
    setConcentration(1,100*.05);
...
/*
	...
	double circleSize= _graph->getAvgEdgeLength()*scale*getScalingFactor();
        ...
        if ( _graph->getNodeSizeNormalization() == GraphWidget::RelativeSize ) {
                ...
                circleSize *= (concentration/totalArea*10);
...
float Node::calculateMetaboliteConcentrations() { 
    ...
    QVector<float> newConc = this->getConcentrations();

    foreach(Edge* edge, this->edgesOut()) {
        float rateConstant = edge->getRateForward();
        for(int i=0; i < concentrations.size(); i++ ) {
            newConc[i] -= (concentrations[i] * rateConstant);
        }
    }


    foreach(Edge* edge, this->edgesIn()) {
        Node* other = edge->sourceNode();
        float rateConstant = edge->getRateReverse();
        QVector<float> otherConc = other->getConcentrations();
        for(int i=0; i < otherConc.size() && i < concentrations.size(); i++ ) {
            newConc[i] += (otherConc[i] * rateConstant);
        }
    }

    if ( this->edgesIn().size() > 0 ) {
        this->setConcentrations(newConc);
    }
}
...
{
    _shape=QPainterPath();
    //int nodesize = computeNodeSize(getTotalConcentration());
    int nodesize = this->getNodeSize();
    float fractionLabeled = getLabeledFraction();

    painter->setBrush(QBrush(_brush));
    painter->setPen(QPen(Qt::black));
    if ( isSelected() )  painter->setPen(QPen(Qt::yellow));
    ...
    //show fraction labeled
        if ( fractionLabeled > 0 ) {
                ...
                QRadialGradient gradient(QPointF(-nodesize/3,-nodesize/3), nodesize);
                gradient.setColorAt(0.2,color2.lighter());
                gradient.setColorAt(0.9,color2);
                gradient.setColorAt(1.0,color2.darker());
                painter->setBrush(gradient);
                painter->setPen(Qt::NoPen);
                double pieSize = fractionLabeled;
                painter->drawPie(-nodesize/2,-nodesize/2,nodesize,nodesize,0,pieSize*360*16);
        }
...
void Node::advance(int step) {
    calculateMetaboliteConcentrations();
    update();
}

double Node::getTotalConcentration() {
    double sum=0;
    for(int i=0; i < concentrations.size(); i++) {
...
double Node::getLabeledFraction() {
    double sum=0; double sumL=0;

    for(int i=0; i < concentrations.size(); i++) {
        sum += concentrations[i];
        if(i>0) sumL += concentrations[i];
    }
    return sumL/sum;
}
