...
void Edge::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *) {
	if (_reversable) {
		drawArrow(painter,-1);	//forward arrow
...
void Edge::wheelEvent ( QGraphicsSceneWheelEvent * event ) {

        float f = this->getRateForward();
        if ( event->delta() > 0 ) { f += 0.1; } else { f -= 0.1; }
        if (f < 0) f=0;
        if (f > 1) f=1;

        this->setRateForward(f);
        this->setRateReverse(f);
}
